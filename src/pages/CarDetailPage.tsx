...
export default function CarDetailPage() {
  ...
  if (!car) return (
    <div className="min-h-screen bg-dark-900 flex flex-col items-center justify-center pt-20 text-center px-6">
      <div className="text-6xl mb-4">🚗</div>
      <h2 className="font-display text-3xl text-white mb-2">Car Not Found</h2>
      <Link to="/inventory" className="btn-brand px-6 py-3 rounded-xl mt-4 text-sm">Back to Inventory</Link>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-dark-900 pt-20">
      <div className="max-w-7xl mx-auto px-6 py-8">
        {/* Back */}
        <Link to="/inventory" className="inline-flex items-center gap-2 text-sm text-gray-400 hover:text-brand-400 transition-colors mb-6">
        ...
        <div className="grid lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-6">
            {/* Image gallery */}
            <div className="rounded-2xl overflow-hidden bg-dark-700 relative">
              {car.images?.length > 0 ? (
                <>
                      ...
                      <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
                        {car.images.map((_, i) => (
                          <button key={i} onClick={() => setImgIndex(i)}
                            className={`w-2 h-2 rounded-full transition-colors ${i === imgIndex ? 'bg-brand-500' : 'bg-white/40'}`} />
                        ))}
                      </div>
                    </>
                  )}
              ...
              {car.images?.length > 1 && (
                <div className="flex gap-2 p-3 overflow-x-auto">
                  {car.images.map((img, i) => (
                    <button key={i} onClick={() => setImgIndex(i)}
                      className={`w-20 h-14 rounded-lg overflow-hidden flex-shrink-0 border-2 transition-colors ${i === imgIndex ? 'border-brand-500' : 'border-transparent'}`}>
            ...
            <div className="glass rounded-2xl p-6">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  <div className="text-sm text-brand-400 font-semibold mb-1 uppercase tracking-wider">{car.make} · {car.category}</div>
                  <h1 className="font-display text-4xl font-black text-white">{car.year} {car.model}</h1>
                  {car.location && <p className="text-gray-400 text-sm flex items-center gap-1 mt-2"><MapPin size={13} />{car.location}</p>}
                </div>
                <div className="text-right">
                  {car.previousPrice && car.previousPrice > car.price && (
                    <div className="text-gray-500 line-through text-sm">#{car.previousPrice.toLocaleString()}</div>
                  )}
                  <div className="font-display text-4xl font-black brand-text">#{car.price.toLocaleString()}</div>
                  {!car.isAvailable && <div className="text-red-400 text-sm font-semibold mt-1">SOLD</div>}
                </div>
              </div>
            </div>
            ...
            {car.description && (
              <div className="glass rounded-2xl p-6">
                <h3 className="font-display text-xl font-bold text-white mb-3">About This Vehicle</h3>
            ...
            {/* Specs */}
            <div className="glass rounded-2xl p-6">
              <h3 className="font-display text-xl font-bold text-white mb-5">Vehicle Specifications</h3>
            ...
            {car.features?.length > 0 && (
              <div className="glass rounded-2xl p-6">
                <h3 className="font-display text-xl font-bold text-white mb-5">Key Features</h3>
          ...
          <div className="space-y-5">
            <div className="glass rounded-2xl p-6 sticky top-24">
              <h3 className="font-display text-xl font-bold text-white mb-1">Enquire About This Car</h3>
              <p className="text-gray-400 text-xs mb-5">Our team will respond within 24 hours</p>
              ...
              {sent ? (
                <div className="text-center py-8">
                  ...
                  <button onClick={() => setSent(false)} className="mt-4 btn-outline-brand px-5 py-2 rounded-xl text-sm">Send Another</button>
                </div>
              ) : (
              ...
              <div className="mt-5 pt-5 border-t border-white/8 space-y-3">
                <a href="tel:+1234567890" className="flex items-center gap-3 text-sm text-gray-400 hover:text-brand-400 transition-colors">
                  <div className="w-8 h-8 rounded-lg bg-brand-500/15 flex items-center justify-center"><Phone size={14} className="text-brand-400" /></div>
                  +1 (234) 567-890
                </a>
                <a href="mailto:sales@autoelite.com" className="flex items-center gap-3 text-sm text-gray-400 hover:text-brand-400 transition-colors">
                  <div className="w-8 h-8 rounded-lg bg-brand-500/15 flex items-center justify-center"><Mail size={14} className="text-brand-400" /></div>
                  sales@autoelite.com
                </a>
              </div>
            </div>
